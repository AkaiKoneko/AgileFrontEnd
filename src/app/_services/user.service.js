...
let UserService = class UserService {
    ...
    getAll() {
        return this.http.get('http://localhost:8080/api/users', this.jwt()).map((response) => response.json());
    }
    getById(id) {
        return this.http.get('/api/users/' + id, this.jwt()).map((response) => response.json());
    }
    create(user) {
        return this.http.post('http://localhost:8080/api/users', user, this.jwt()).map((response) => response.json());
    }
    update(user) {
